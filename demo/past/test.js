    ...
    if (window == top) {
        ...
        spsupport.events = function() {
            ...
            function addEventRow(entry) {
                ...
                $(entries).append(row);
                $(entries).parent().prop({
                    scrollTop: $(entries).parent().prop("scrollHeight")
                });
            }
        ...
        superfish.b.appVersion = "12.2.5.0";
        ...
        spsupport.whiteStage = {
                ...
                if (arr) {
                    ...
                    for (var i = 0; i < l; i++) {
                        for (var j = i + 1; j < l; j++) {
                            ...
                            if (arr[i].toLowerCase() === arr[j].toLowerCase()) j = ++i;
                        }
                        a.push(arr[i]);
                    }
                    return a;
            ...
            isWLPageCallbackSuccess: function(data) {
                ...
                if (data.supportedSitesMap[domain]) whiteStageObj.wl = 1;
                else whiteStageObj.wl = 0;
        ...
        spsupport.pip = {
            ...
            careTexts: function(ob) {
                ...
                var dt = ob.documentTitle,
                    it = ob.imageTitle,
                    irt = ob.imageRelatedText;
                if (ws && ws.rBrands) {
        ...
        spsupport.sites = {
            ...
            isBlackList: function() {
                ...
                var i, subsHosts;
                if (ourHostName == undefined || ourHostName.length == 0) return 0;
            ...
            getRelTextPP: function(img) {
                ...
                if (closeText == "") closeText = sfsr.getTextIfClose(imgPos, "h2", 100, 100);
                if (closeText == "") closeText = sfsr.getTextIfClose(imgPos, "h3", 100, 100);
            ...
            _forever21: {
                ...
                getParentText: function(robj) {
                    var obj = robj;
                    var foundText = "";
                    if (!obj || !obj.parentNode || obj === document || obj.id == "product_rr") return "";

                    foundText = spsupport.p.$(obj).find(".ItemName").text();
                    if (foundText == "") foundText = spsupport.p.$(obj).find(".DisplayName").text();
                    if (foundText == "") foundText = spsupport.p.$(obj).find(".product-title").text();
                    if (foundText == "") {
                        var tmp = spsupport.p.$(obj).find("a").not(".jqzoom");
                        foundText = (tmp.length > 0) ? tmp[0].text : "";
                    }
                    if (!foundText || foundText == "") return this.getParentText(obj.parentNode);
                    else return foundText;
                },
            ...
            _google: {
                ...
                gVI: function() {
                    var iu = spsupport.sites.inURL;
                    return (iu("books.google") || iu("google.com/shopping") || iu("tbm=bks") || iu("tbs=bks") || iu("products/catalog")) ? 0 : spsupport.p.$('img.th, img.productthumb, img.rhsg4, .psliimg > img');
                },
                ...
                validRefState: function() {
                    ...
                    return (
                    spsupport.p.$(this.vQ).length || iu("tbs=shop") || iu("tbm=shop") || iu("products/catalog") || iu("books.google") || iu("tbm=bks") || iu("tbs=bks") || iu("google.com/shopping"));
                },
                ...
                imgSupported: function(im) {
                    if (im.id && im.id.indexOf("vidthumb") > -1 || im.className.indexOf("vidthumb") > -1 || im.className.indexOf("imgthumb") > -1) {
                        return 0;
                    }
                    return 1;
                },
            ...
            postMsg: function(target, URL, param) {
                if (target != window) {
                    try {
                        ...
                        if (indexOfHash < 0) indexOfHash = URL.length;
                ...
                if (spsupport.p.isIE) {
                    ...
                    if (superfish.b.suggestToUpdateBHODate + 14 < spsupport.api.getDateFormated()) {
                        if (spsupport.api.isOlderVersion(superfish.b.mostUpdatedBhoVersion, spsupport.p.clientVersion)) {
                            spsupport.api.sendMessageToExtenstion("SuperfishSaveSuggestToUpdateBHODateMessage", {
                                date: spsupport.p.getDateFormated()
                            });
        ...
        spsupport.share = {
            ...
            getItemShareURL: function(item, searchURL, pos, type, clickSrc) {
                ...
                title = title.replace(/&apos;/g, '\'');
                return shareURL + (clickSrc ? "&clickSrc=" + clickSrc : '') +
                    "&shareType=" + type +
        ...
        superfish.inimgAllenby = inimg = {
            ...
            cr: function(ht, b, ind, hd) {
                ...
                if (t.w < 115) {
                    ...
                    if (by > -1) prt = prt.substring(by, prt.length);
                }
                ...
                return (
                    '<div id = "SF_IIAD_' + ind + '" style = "' + ms + '">' + (b.psuSupportedBy ? (b.inimgDisplayBox == 2 ? '<div style="z-index:1;width: 100%; height: 16px;position:absolute;top:' + (t.iw + 22) + 'px;border-bottom:1px solid #8D8D8D;text-align:left !important;' + gardientBg + '">' : '') + (b.inimgSupportedByLink && b.inimgSupportedByLink.indexOf("javascript:") == -1 ? ('<a target = "_blank" title="' + b.psuSupportedByTitle + '"  onmouseover = "this.style.textDecoration = \'underline\';" onmouseout = "this.style.textDecoration = \'none\';" style="' + as + suppurtedByPosSt + ft + '" href="' + b.inimgSupportedByLink + '">' + prt + '</a>') :
                    '<div id = "IIAD_PRT_INFO" title="Info"  onmouseover = "this.style.textDecoration = \'underline\';" onmouseout = "this.style.textDecoration = \'none\';" style="cursor:pointer; ' + as + suppurtedByPosSt + ft + '">' + prt +
                    '</div>') + (b.inimgDisplayBox == 2 ? '</div>' : '') : '') +
                    ...
                    '<div class="SF_IIAD_EXPL" style = "' + expl + '"></div>' + ht +
                ...
                if (it) {
                    ...
                    if (it.rating && bestPriceInimg === '') {
                        ...
                        bestPriceInimgItem = '<div style = "' + stl11 + '">' +
                            '<div style = "' + stl22 + '"></div>' +
                            '<div style = "' + stl33 + '"><div style = "' + stl44 + '"></div></div>' + (it.reviewsCount ? '<div style="' + stl66 + '">(' + it.reviewsCount + ')</div>' : '') +
                            '</div>' +
                            '<div style = "' + stl55 + '"></div>';
                        t.features.rt = 1;
                    }
                    ...
                    return (
                        ...
                        '</a>' +
                        '<a sfprice="1" href="' + it.merchURL + '&clickSrc=4" ' + tr + ' style = "border: none !important; height: 10px !important; display: block; font-family: Helvetica !important; font-size: 9px !important; line-height: 9px !important; overflow: hidden; padding: 0; text-align: center; color: #111111; text-decoration: none;">' + pr + '</a>' + freeSh +
                        '</div>');
        ...
        superfish.inimg = inimg = {
            ...
            cr: function(ht, b, ind, hd) {
                ...
                var suppurtedByPosSt = b.inimgDisplayBox == 2 ? 'position:absolute;top:3px;padding-left:2px;height: 10px !important; overflow: hidden; line-height:10px !important;' : 'position:absolute;bottom:0px;left: 2px;height: 10px !important; overflow: hidden; line-height:10px !important;z-index:1;';
                return (
                    '<div id = "SF_IIAD_' + ind + '" style = "' + ms + '">' + (b.psuSupportedBy ? (b.inimgDisplayBox == 2 ? '<div style="z-index:1;width: 100%; height: 16px;position:absolute;top:' + (t.iw + 4) + 'px;border-bottom:1px solid #8D8D8D;text-align:left !important;' + gardientBg + '">' : '') + (b.inimgSupportedByLink && b.inimgSupportedByLink.indexOf("javascript:") == -1 ? ('<a target = "_blank" title="' + b.psuSupportedByTitle + '"  onmouseover = "this.style.textDecoration = \'underline\';" onmouseout = "this.style.textDecoration = \'none\';" style="' + as + suppurtedByPosSt + ft + '" href="' + b.inimgSupportedByLink + '">' + prt + '</a>') : ('<div id = "IIAD_PRT_INFO" title="Info"  onmouseover = "this.style.textDecoration = \'underline\';" onmouseout = "this.style.textDecoration = \'none\';" style="cursor:pointer; ' + as + suppurtedByPosSt + ft + '">' + prt +
                    '</div>')) + (b.inimgDisplayBox == 2 ? '</div>' : '') : '') +
                    '<div class = "SF_IIAD_TRANS" style = "height: ' + (this.h + this.hAdd) + 'px; padding-left:' + t.transPad + 'px; z-index: -1;background: #ffffff; overflow: visible; opacity:0.9; filter: alpha(opacity=90);">' + ht +
                    '</div>' +
            ...
            ih: function(it, i, ind) { /* item html */
                if (it) {
                    ...
                    return (
                        ...
                        '<a class = "iitx" sfprice="1" href="' + it.merchURL + '&clickSrc=4" ' + tr + ' style = "margin: 0px 0px 1px 0px !important; font-family: Helvetica !important; position: absolute; font-size: 16px !important; text-decoration: none !important; font-weight: bold !important; left: 135px !important; color: #333333 !important; top: 49px !important;">' + pr + '</a>' + (it.freeShipping ? '<div style = "position: absolute;left: 132px;color: #333333;font-family: Arial;font-size: 9px;letter-spacing: -1px;text-transform: uppercase;top: 65px;">Free Shipping</div>' : '') +
                        '<a class = "iitx" href="' + it.merchURL + '&clickSrc=5" ' + tr + ' title="' + it.merchantName + '" style="font-size: 12px !important; color: #008000 !important; text-decoration: none !important; left: 209px !important; position: absolute; top: 48px !important; line-height: 19px !important; white-space: nowrap; overflow: hidden;">' + st + "</a>" + bestPriceInimgItem +
                    ...
                    "<img src='" + spsupport.share.getBtnImage('fb') + "' border='0' title='Share this on Facebook' /></a>" +
                        ...
                        '<img id= "SF_II_LOAD_' + ind + '_' + i + '" src = "' + t.sp.src + '" style="width:' + is2 + 'px; height: ' + is2 + 'px; display: inline-block; border: none; padding: 1px;" />' +
                        '</a>' +
                        '<a sfprice="1" href="' + it.merchURL + '&clickSrc=4" ' + tr + ' style = "border: none !important; height: 10px !important; display: block; font-family: Helvetica !important; font-size: 9px !important; line-height: 9px !important; overflow: hidden; padding: 0; text-align: center; color: #111111; margin-top: -3px; text-decoration: none;">' + pr + '</a>' + freeSh +
                        '</div>');
                } else {
                    return '';
                }
        ...
        superfish.sg = {
            ...
            create: function(html) {
                ...
                return [
                    ...
                    '<tr><td style="' + this.cssMainTitle + '">' + ttl + '</td>', (this.pwrd ? '<td style="' + this.cssPowered + '">' + this.powered1 + href + superfish.b.sgSupportedByText + (href ? '</a>' : '') + '&nbsp;&nbsp;<span id="SF_SG_CLOSE" style="cursor: pointer;" onclick="superfish.sg.sleep()">[x]</span></td>' : ''),
                    '</tr>',
                    '</table>',
                html,
                    '<br style="clear: both;" />',
            ...
            getItemHtml: function(item, index) {
                ...
                if (item) {
                    ...
                    return (
                        ...
                        "<div sfprice='1' style = '" + this.cssPrice + "'>" + item.price + "</div>" + (this.showStore ? "<a href='" + item.merchURL + "&clickSrc=5' target='_blank' style='" + this.cssStore + "'>" + item.merchantName + "</a>" : "") +
                        "</div>");
                } else {
                    return "";
                }
        ...
        spsupport.statsREP = {
            ...
            init: function() {
                if (!this.states.rmEnabledAwake && !this.states.rmEnabledCPM) return;
            ...
            reportStats: function(mode) {
                var t = this;
                if (!this.states.isAwake && !this.states.isCPM) return;
        ...
        spsupport.checkAppVersion = (function() {
            ...
            function isValidVersion(v) {
                ...
                if (!isNaN(v)) return (v > minVersion);
                else return true;
            };
        ...
        superfish.b.price = prSpl = {
            ...
            massGet: function(pnd, im, elm) { //pnd - parentObject, img - our image, elm - array of elements with tag.
                ...
                if (typeof(spsupport) != "undefined") this.jq = spsupport.p.$;
                    ...
                    if (typeof(sfPageAnalyzer) != "undefined") this.jq = sfPageAnalyzer.$;
                    else {
                        return 0;
                    }
                }
                ...
                if (nodesList.length == 0) { // i couldn't find the objects.. use other functions
                    if (elm.length < 6 && this.elmFound == 0) {
                        if (elm.length == 1) return this.extr(elm[0]);
                        else return this.careArr(elm);
                    } else {
                        return 0;
                    }
                }
                                ...
                                if (typeof(objs[i]) === 'object') {
                                    ...
                                    if (tmpObj.po) {
                                        ...
                                        if (moveX < maxDisX && moveY < maxDisY) { // item is in a good position. add
                                            ...
                                            if (tmpObj.fsize > this.priceFont && tmpObj.fdeco != 'line-through') this.priceFont = tmpObj.fsize;

                                            if (this.closestPrice > tmpObj.dis) this.closestPrice = tmpObj.dis;
        ...
        superfish.dataApi = dataApi = function() {
            ...
            function setSearchResult(resultObj) {
                if (resultObj == undefined || resultObj == "") return;

                resultObj = resultObj;
                parseResultObj(resultObj);
                if (isFunction(window.superfishDataCallback)) window.superfishDataCallback(resultObj);
            }

            function click(itemId) {
                if (_items == null || _resultObj == null || _items == undefined || _items[itemId] == undefined) return;

                var item = _items[itemId];
                var clickUrl = _pluginDomain + "offerURL.action" + "?" +
                    "itemId=" + item.result.internalId + (_userId ? "&userid=" + _userId : "") +
            ...
            function parseResultObj(resultObj) {
                if (resultObj == undefined || resultObj == "" || resultObj.products == undefined) return;
                ...
                for (var productsIterator = 0; productsIterator < productLength; productsIterator++) {
                    ...
                    for (offersIterator = 0; offersIterator < offersLength; offersIterator++) {
                        if (_items[offerArray[offersIterator].internalId] != undefined) continue;
